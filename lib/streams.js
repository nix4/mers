...
function BufferedJSONStream(meta, single) {
    Stream.call(this);
    this.response = _u.extend({payload:[], status:0}, meta);
    this.writable = true;
    this._done = false;
    this._single = single;
...
BufferedJSONStream.prototype.end = BufferedJSONStream.prototype.destroy = function () {
    if (this._done) return;
    this._done = true;
...
function PayloadlessBufferedJSONStream(meta, single) {
    BufferedJSONStream.call(this);
    this.response = [];
    this.writable = true;
    this._done = false;
    this._single = single;
...
PayloadlessBufferedJSONStream.prototype.__proto__ = BufferedJSONStream.prototype;

PayloadlessBufferedJSONStream.prototype.write = function (doc) {
    this.response.push(doc);
    return true;
}

module.exports.PayloadlessBufferedJSONStream = PayloadlessBufferedJSONStream;
...
var streamNameMap = {PayloadlessBufferedJSONStream: PayloadlessBufferedJSONStream, BufferedJSONStream: BufferedJSONStream};
function getStream(name){
    return streamNameMap[name];
}
